...
export default function Page({ data }) {
  return (
    <>
      {/* ... (kode navigasi dan header) */}

      <main className="flex min-h-screen flex-col p-20 bg-white w-full h-screen bg-center bg-cover">
        <h1 className="text-black font-extrabold text-7xl mb-8">Mau belanja apa hari ini?</h1>

        <div className="grid grid-cols-3 gap-4">
          {data.map((item) => (
            <div key={item._id} className="cards col-span-2 flex grid-cols-2 gap-6">
              <div className="card block rounded-lg bg-custom-F2F2F2 p-6 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] w-full">
                <div className="flex justify-between">
                  <div className="flex">
                    <Image
                      className="pr-2 aspect-w-1 aspect-h-1 w-40 h-40"
                      src={item.image}
                      alt={item.name}
                      width={160}
                      height={160}
                    ...
                    <div className="flex flex-col m-0">
                      <h5 className="mb-2 text-xl font-bold leading-tight text-black">{item.name}</h5>
                      <p className="mb-4 text-base text-slate-500 font-medium">{item.category}</p>
                      <p className="mt-8 mb-0 text-3xl text-black font-medium">{`Rp${item.price} / ${item.unit}`}</p>
                    </div>
                  </div>
                  <a href='/checkout'>
                    <button
                      type="button"
                      className="self-end text-black rounded bg-primary px-6 pb-2 pt-2.5 text-xl font-extrabold uppercase leading-normal shadow-[0_4px_9px_-4px_#3b71ca] transition duration-150 ease-in-out hover:bg-primary-600 hover:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] focus:bg-primary-600 focus:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] focus:outline-none focus:ring-0 active:bg-primary-700 active:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] dark:shadow-[0_4px_9px_-4px_rgba(59,113,202,0.5)] dark:hover:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.2),0_4px_18px_0_rgba(59,113,202,0.1)] dark:focus:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.2),0_4px_18px_0_rgba(59,113,202,0.1)] dark:active:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.2),0_4px_18px_0_rgba(59,113,202,0.1)]"
                      data-te-ripple-init
...
// Misalnya, data diambil dari API atau sumber lainnya
export async function getStaticProps() {
  ...
  return {
    props: {
      data: data.data, // Sesuaikan dengan struktur data yang diterima
    },
  };
}
